...
describe('convert_color tool', () => {
  it('converts a hex color to all formats', async () => {
    ...
    const result = await toolHandler(tool)({ color: '#3b82f6' }, {} as never);
  ...
  it('applies alpha when provided', async () => {
    ...
    const result = await toolHandler(tool)({ color: '#3b82f6', alpha: 0.5 }, {} as never);
  ...
  it('returns an error result for unparseable input', async () => {
    ...
    const result = await toolHandler(tool)({ color: 'not-a-color' }, {} as never);
