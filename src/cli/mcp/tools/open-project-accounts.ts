...
export function registerOpenProjectAccountsTool(server: McpServer): RegisteredTool {
  return server.registerTool(
    ...
    {
      description:
        "Focus the Barnacles app window and navigate it to a project's Accounts tab, so the user can view a password themselves. Passwords are never returned to the assistant. Use list_projects first to find the project ID.",
      inputSchema: {
